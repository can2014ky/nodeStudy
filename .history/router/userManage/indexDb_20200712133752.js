...
router.post('/user/create', (req, res) => {
    new userManagerDataProcess(req.body).save( (err) => {
            if (err) {
                return res.status(500).send('server error');
            }
            res.redirect('/');
...
router.get('/user/edit', (req, res) => {
    const { id } = req.query;
    userManagerDataProcess.findById(id, (err, data) => {
        if (err) {
            return res.status(500).send('server error');
        }
...
router.post('/user/update', (req, res) => {
    const data = req.body;
    userManagerDataProcess.findByIdAndUpdate(data.id, data, (err) => {
        if (err) {
            return res.status(500).send('server error');
        }
        res.redirect('/');
...
router.get('/user/delete', (req, res) => {
    const { id } = req.query;
    userManagerDataProcess.findByIdAndRemove(id, (err) => {
        if (err) {
            return res.status(500).send('server error');
        }
        res.redirect('/');
