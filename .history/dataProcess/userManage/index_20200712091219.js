...
const updateUser = (updateUserInfo, callback) => {
    fs.readFile(dbPath, 'utf8', (err, data) => {
        if (err) {
            ...
            const resultData = JSON.parse(data).userInfoList;
            updateUserInfo.id = parseInt(updateUserInfo.id);
            const userInfo = resultData.find((item) => {
                return item.id === updateUserInfo.id;
            });
...
const deleteUser = (id, callback) => {
    fs.readFile(dbPath, 'utf8', (err, data) => {
        if (err) {
            ...
            const resultData = JSON.parse(data).userInfoList;
            const deleteIndex = resultData.findIndex((item) => {
                return item.id === parsInt(id);
            });
            resultData.splice(deleteIndex, 1);
            const obj = JSON.stringify({
                userInfoList: resultData,
            });
            fs.writeFile(dbPath, obj, (err) => {
